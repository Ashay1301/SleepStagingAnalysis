...
def predict(
    ...
    for fold_idx in range(config["n_folds"]):
        ...
        acc_list.append(s_acc*100)
        f1_list.append(s_f1_score*100)

        logger.info("n={}, acc={:.1f}, mf1={:.1f},acc_list{:},f1_list{:}".format(
            len(s_preds),
            s_acc*100.0,
            s_f1_score*100.0,
            acc_list,
        ...
        print("Confusion matrix Test: ",s_cm[1][1])
    ...
    with open("Vis_data/acc_data.txt", "w") as f:
        for s in acc_list:
            f.write(str(s) +"\n")
    f.close()
    with open("Vis_data/f1_data.txt", "w") as f:
        for s in f1_list:
            f.write(str(s) +"\n")
    f.close()
    with open("Vis_data/CM_data.txt", "w") as f:
        for s in cm:
            f.write(str(s) +"\n")
    f.close()
