...
def predict(
    ...
    for fold_idx in range(config["n_folds"]):
        ...
        pre = skmetrics.precision_score(y_true=s_trues, y_pred=s_preds, labels=[0,1,2,3,4], pos_label=1, average='macro', sample_weight=None, zero_division='warn')
        recall = skmetrics.recall_score(y_true=s_trues, y_pred=s_preds, labels=[0,1,2,3,4], pos_label=1, average='macro', sample_weight=None, zero_division='warn')
        logger.info("n={}, acc={:.1f}, mf1={:.1f}, acc_list={:}, pre={:.1f}, recall={:.1f}".format(
            len(s_preds),
            s_acc*100.0,
            s_f1_score*100.0,
            acc_list,
    ...
    pre = skmetrics.precision_score(y_true=trues, y_pred=preds, labels=[0,1,2,3,4], pos_label=1, average='macro', sample_weight=None, zero_division='warn')
    recall = skmetrics.recall_score(y_true=trues, y_pred=preds, labels=[0,1,2,3,4], pos_label=1, average='macro', sample_weight=None, zero_division='warn')
    logger.info("")
    logger.info("=== Overall ===")
    print_n_samples_each_class(trues)
    logger.info("n={}, acc={:.1f}, mf1={:.1f}, pre={:.1f}, recall={:.1f}".format(
        len(preds),
        acc*100.0,
        f1_score*100.0,
        pre*100.0,
    ...
    #logger.info("Accuracy List: {}".format(metrics[7]))
